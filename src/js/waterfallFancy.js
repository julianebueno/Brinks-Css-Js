...
async function Draw(){
  ...
  if(!t){
    ...
    genTerrain = () =>{
      ...
      oy = -25
      oz = 0
      def = typeof terrain != 'undefined' 
      a = Array(cl*rw*br).fill().map((v,i)=>{
        if(!(i%cl)){
          oyv /= 1.1
          oyv+=(((i+99)**3.1%1)-.35)*2
          oy += oyv
        ...
        channel = C(Math.PI*2/cl*(i%cl)+ox)*3**4/6 * S(t*2)
        X = ((i/cl/rw|0)-br/2+.5)*sp + oz
        Y = (((i/cl|0)%rw)-rw/2+.5)*sp + oy - channel
        Z = ((i%cl)-cl/2+.5)*sp + ox
        return [X,Y,Z,def ? terrain[i][3] : 0]
      })
      terrain = a
      iPv = S(t)/2
    }
    ...
    spawnSparks = (X,Y,Z) =>{
      for(m=3;m--;){
        vx = S(p=Math.PI*2*Rn()) * S(q=Rn()<.5?Math.PI-Math.PI/2*Rn()**.5:Math.PI/2*Rn()**.5) * iSparkv
        vy = C(q) * iSparkv
        vz = C(p) * S(q) * iSparkv
        sparks = [...sparks, [X,Y,Z,vx,vy,vz,1]]
      }
    }
    bounding = subDividedDodecahedron(75,1,1)
  }
  
  oX=0, oY=0, oZ=Math.min(110,Math.max(75,(.3+C(t/2))*300))
  Rl=C(t/3)/4, Pt=-Math.PI/6-C(t)/3, Yw = C(t/4)*4-Math.PI/1.5
  ...
  terrain.map((v,i)=>{
    ...
    if(Z>0){
      ...
      if(i%cl && i/cl|0){
        ...
        R(Rl,Pt,Yw,1)
        if(Z>0) x.lineTo(...Q())
        col2 = `hsla(${v[3]*250+t*50},99%,${50+(v[3]*200)**1/3}%,${Math.min(1,v[3]*9+.2)})`
        stroke('', col2, 5, false)
        P.map(q=>{
          X1_ = q[0]
          Y1_ = q[1]
          ...
          ay = (Y1+Y2+Y3+Y4) / 4
          az = (Z1+Z2+Z3+Z4) / 4
          X2_ = X1_ + q[3] * (1+anticipation)
          Y2_ = Y1_ + q[4] * (1+anticipation)+.15
          Z2_ = Z1_ + q[5] * (1+anticipation)
          if((d = Math.hypot(ax-X2_,ay-Y2_,az-Z2_))<sp*2.5){
            a = [
              [X1,Y1,Z1],
              [X2,Y2,Z2],
              [X3,Y3,Z3],
  ...
  stars.map((v,i)=>{
    ...
    R(Rl, Pt, Yw, 1)
    if(Z>0){
      s = Math.min(3e3,7e3/Z*v[3])
      l = Q()
      x.drawImage(starImgs[2+i%7].img,l[0]-s/2,l[1]-s/2,s,s)
    }
    v[3]-=.015
  })
